...
export function HeroConfigForm({ config, onChange, hasWeddingDate = true, weddingNameId }: HeroConfigFormProps) {
  ...
  const allVariants = [
    ...
    { value: 'old-money', label: 'Old Money', description: 'Parallax hero with crest, border frame, and antique gold rules', requiresImage: true }
  ]
    ...
    <div className="space-y-6">
      ...
      {config.variant === 'old-money' && (
        <>
          <div>
            ...
            <div className="flex gap-2">
              {(['top', 'center', 'bottom'] as const).map((pos) => (
                <Button
                  key={pos}
                  variant={(config.textPosition ?? 'bottom') === pos ? 'default' : 'outline'}
                  size="sm"
                  className="flex-1"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {t('config.imageBrightness')}: {config.imageBrightness ?? 75}%
            </label>
            <input
              type="range"
              min="30"
              max="100"
              value={config.imageBrightness ?? 75}
              onChange={(e) => onChange('imageBrightness', parseInt(e.target.value))}
              className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
            />
            <div className="flex justify-between text-xs text-gray-500 mt-1">
