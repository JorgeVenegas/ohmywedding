...
create table "invitation_opens" (
  ...
  "is_owner_view" boolean default false, -- Flag to exclude owner testing views
...
-- Public insert (for tracking pixel)
...
-- Allow insert through functions (service role)
...
returns trigger as $$
begin
  -- Update the guest_group with first_opened_at (if not set) and increment open_count
  update guest_groups
  set 
