...
-- Mirrors Stripe coupon/promotion code objects locally for
-- superadmin management, tracking, and redemption history.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS coupons (
  ...
  discount_value  INTEGER NOT NULL,           -- percent (0-100) or amount in centavos
  currency        TEXT NOT NULL DEFAULT 'mxn', -- only relevant for fixed discounts
...
-- Promotion Codes (user-facing code strings, maps to Stripe Promotion Code objects)
CREATE TABLE IF NOT EXISTS coupon_promotion_codes (
  id                       UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  coupon_id                UUID NOT NULL REFERENCES coupons(id) ON DELETE CASCADE,
  stripe_promotion_code_id TEXT UNIQUE NOT NULL,
  code                     TEXT UNIQUE NOT NULL,  -- user-facing code e.g. WEDDING2026
  is_active                BOOLEAN NOT NULL DEFAULT true,
  max_redemptions          INTEGER,               -- NULL = unlimited (inherits coupon limits)
  times_redeemed           INTEGER NOT NULL DEFAULT 0,
  first_time_transaction   BOOLEAN NOT NULL DEFAULT false,
  minimum_amount_cents     INTEGER,               -- minimum order for code to apply
  expires_at               TIMESTAMPTZ,
  created_at               TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- Superadmins: full access to all coupon tables
...
-- Users: can read active promotion codes (for validation) but nothing else
