...
import { LayoutDashboard, Users, ShieldCheck, Headphones } from "lucide-react"
...
const ConsultationSection = dynamic(() => import("@/components/landing/consultation-section").then(m => ({ default: m.ConsultationSection })), { ssr: false })
...
function AuthCodeHandler() {
  ...
  useEffect(() => {
    if (code) {
      const callbackUrl = `/auth/callback?code=${code}&redirect=/couples`
...
export default function CouplesLandingPage() {
  return (
    <main className="min-h-screen bg-[#420c14] overflow-x-hidden">
      ...
      <ConsultationSection ns={NS} calSlug={process.env.NEXT_PUBLIC_CALCOM_COUPLES_SLUG ?? 'couples-intro'} />
      ...
      <AudienceTeaserSection
        ns={NS}
        sectionKey="plannerTeaser"
        ctaHref="/planners"
        icons={[
          <LayoutDashboard key="dashboard" className="w-5 h-5 sm:w-6 sm:h-6" />,
          <Users key="collab" className="w-5 h-5 sm:w-6 sm:h-6" />,
          <ShieldCheck key="branding" className="w-5 h-5 sm:w-6 sm:h-6" />,
          <Headphones key="support" className="w-5 h-5 sm:w-6 sm:h-6" />,
        ]}
      />
      <LuxuryFooter />
    </main>
