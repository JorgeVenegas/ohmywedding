...
// GET: Fetch all menu assignments for a wedding
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('*, guests(id, name), menus(id, name)')
...
// POST: Assign a menu to guest(s) — supports bulk by group or table
export async function POST(request: Request) {
  try {
    ...
    const { menu_id, guest_ids, group_id, table_id } = body
    ...
    if (group_id) {
      ...
      if (groupGuests) targetGuestIds = [...targetGuestIds, ...groupGuests.map(g => g.id)]
    }
    ...
    if (table_id) {
      ...
      if (tableAssignments) targetGuestIds = [...targetGuestIds, ...tableAssignments.map(a => a.guest_id)]
    }

    targetGuestIds = [...new Set(targetGuestIds)]
    if (targetGuestIds.length === 0) return NextResponse.json({ error: "No guests to assign" }, { status: 400 })
...
// DELETE: Remove menu assignment for a specific guest
export async function DELETE(request: Request) {
  try {
    const { searchParams } = new URL(request.url)
    const weddingId = searchParams.get('weddingId')
    const guestId = searchParams.get('guestId')
    if (!weddingId || !guestId) return NextResponse.json({ error: "weddingId and guestId are required" }, { status: 400 })
