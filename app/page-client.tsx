...
function LoggedInRedirector() {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      router.replace(data ? '/superadmin' : '/admin')
    }
    check()
  }, [router])
  ...
  return (
    <main className="relative h-dvh w-full overflow-hidden bg-[#420c14]">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-1/2 z-10 h-px bg-[#DDA46F]/20 sm:inset-y-0 sm:inset-x-auto sm:left-1/2 sm:top-0 sm:h-full sm:w-px">
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-[#420c14] border border-[#DDA46F]/40 flex items-center justify-center">
          <span className="text-[#DDA46F] text-[9px] tracking-[0.15em] uppercase font-light">or</span>
        </div>
      </div>
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-20">
        ...
        <div className="flex flex-col items-center px-4 pb-4 sm:pb-6 text-center">
          ...
          <h1 className="font-serif font-light text-2xl sm:text-3xl md:text-4xl text-[#f5f2eb] mb-2">
            {t('audienceChooser.title')}
          </h1>
          <p className="text-[#f5f2eb]/55 text-xs sm:text-sm tracking-[0.15em] uppercase">
            {t('audienceChooser.hint')}
          </p>
        </div>
      </div>
